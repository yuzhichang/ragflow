...
func (m *EinoChatModel) name() string {
	if m == nil || len(m.chain) == 0 || m.chain[0].ModelName == nil {
		return ""
	}
...
func (m *EinoChatModel) Generate(ctx context.Context, msgs []*schema.Message, opts ...model.Option) (*schema.Message, error) {
	...
	// Terminal: every entry failed. Cache the outcome and rotate the cursor so
	// the next attempt starts on a different entry instead of always spending
	// the primary's error budget first.
	m.mu.Lock()
...
func (m *EinoChatModel) Stream(ctx context.Context, msgs []*schema.Message, opts ...model.Option) (*schema.StreamReader[*schema.Message], error) {
	...
	go func() {
		...
		for i := 0; i < len(m.chain); i++ {
			...
			sentBefore := sentAny
			sendMu.Unlock()
			err := cm.ModelDriver.ChatStreamlyWithSender(ctx, *cm.ModelName, internalMessage, cm.APIConfig, chatCfg, nil, sender)
			if err == nil {
		...
		// Terminal: sweep exhausted. Cache + rotate, mirroring Generate.
		m.mu.Lock()
		m.sweep.failedAt = time.Now()
		m.sweep.err = lastErr
		m.cursor = (start + 1) % len(m.chain)
		...
		common.Debug("models: eino stream response error",
			zap.String("model", modelNameOf(m.chain[0])), zap.Error(lastErr))
