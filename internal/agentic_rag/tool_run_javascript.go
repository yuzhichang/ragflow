...
const runJavascriptToolDescription = "Execute a self-contained ECMAScript 5.1 (ES5.1) snippet and return its standard output.\n\n" +
	"## When to use it (USE IS OPTIONAL - most questions need no code at all)\n" +
	"Use it ONLY for computation over values you already hold: arithmetic on numbers, date/interval math, sorting or ranking lists, counting and aggregation, string/JSON parsing and transformation. The snippet must DERIVE its output from input values.\n" +
	"Do NOT use it to think out loud. Reasoning, weighing evidence, comparing candidates, or deciding what to retrieve next belong in the `think` tool or in your own reply - never here. Anti-patterns that waste a turn: a snippet that is mostly `//` comments narrating your reasoning; a snippet whose only statement is `console.log(\"the answer is X\")` printing a constant you already knew; a snippet that echoes back a conclusion reached elsewhere. If you cannot name the INPUT VALUES the code transforms, you have no reason to call this tool.\n\n" +
...
func (t *RunJavascriptTool) InvokableRun(ctx context.Context, argumentsInJSON string, _ ...einotool.Option) (string, error) {
	return guardedToolRun(ctx, runJavascriptToolName, t.invokableRun, argumentsInJSON)
}

func (t *RunJavascriptTool) invokableRun(ctx context.Context, argumentsInJSON string) (string, error) {
	...
	// Interrupt the interpreter when the request context is done
	// (deadline/cancellation). goja returns an InterruptedError at the next
	// instruction boundary, so e.g. `while(true){}` ends when the run-level
	// timeout cancels the agent's context — there is intentionally no separate
