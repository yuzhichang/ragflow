...
//
// The degradation this makes visible is otherwise silent until a model is actually used:
// an unavailable counter falls back to the calibrated cl100k estimate, which under-counts
// some tokenizers (see embedding_token_limits.md), and the only other sign of it is the
// per-embedder warning in the ingest path.
...
// ids and unloadable assets all fall back to cl100k_base — the caller then has
// to compensate with a calibrated ratio (see Limiter).
...
func (c *Calibration) Stats(key string) (ratio float64, samples, limitRejects int, ok bool) {
	...
	return c.RatioUpper(key), e.samples, e.limitRejects, true
}
