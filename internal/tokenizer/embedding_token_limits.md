...
- **How it is verified.** `scripts/gen_tokenizer_oracle.py` generates fixtures and
  ...
  (**4 families × 35 samples, all equal**); a second tier holds *every* counter to the same
...
what decides whether a calibrated ratio has to be applied.
...
| `bert-wordpiece` | 4 | 4 | **only the tag** (`BAAI/bge-large-zh-v1.5`, `netease-youdao/bce-embedding-base_v1`, …) |
| `llama-bpe` | 3 | 3 | **only the tag** (`mistral/mistral-embed`, two `nvidia/llama-nemoretriever-*`) |
...
|---|---|---|---|---|
| `bert-wordpiece` | `BAAI/bge-large-en-v1.5/vocab.txt` | `do_lower_case`, `strip_accents`, `handle_chinese_chars`, `max_input_chars_per_word`, the unk token | yes, parsed from the model's `tokenizer.json` | **partly** - the fixture is `lowercase=true`; no cased model is in the corpus |
...
3. **Add the fixture line** to `ORACLES` in `scripts/gen_tokenizer_oracle.py`, generate it,
   and run `TestCountersMatchOracle`. Passing means 341/341 (or inside the flagged bounded
   class); anything else is a bug in the counter, not in the model.
...
perform no network I/O. A missing asset is a WARNING, not a build failure, because the Go
side degrades to the calibrated fallback (the cl100k table itself is the one asset that
still comes from `download_deps.py`, which fetches it from the OpenAI blob).
...
runtime image), and when a declared tag cannot load its asset the embedder logs
`embedding tokenizer "<id>" is declared for <model> but unavailable; counting with the
calibrated fallback` once per embedder instead of silently counting differently.
...
- the ingestor logs an **availability report once at startup** -
  ...
  `MODEL_ASSETS_DIR`. Without it, a deployment that ships no assets looks healthy and
  quietly counts with the calibrated fallback.
...
for m in bge-m3 bge-large-en qwen3-embedding e5-mistral cl100k; do
  python3 scripts/gen_tokenizer_oracle.py --model $m --out /tmp/tokenizer_oracle/$m.json --fuzz 300
done
TOKENIZER_ORACLE_DIR=/tmp/tokenizer_oracle ./build.sh --test -run TestCountersMatchOracle ./internal/tokenizer/
```

Current state: **341 samples per counter, all five counters passing** - four of them
...
```bash
python3 scripts/gen_tokenizer_oracle.py --model bge-m3 \
...
Measured result on the current corpus (341 samples):
...
| A finite corpus, however adversarial: 341 samples per counter is evidence, not proof | the corpus is seeded and regenerated from the model on demand (`--fuzz N`), so coverage is a dial rather than a constant, and the runtime oracle covers the rest |
...
- **Combining marks are where that subset is measurably short**, and the oracle test
  ...
  is still compared exactly, which is 158/341 for `xlmr-spm` and 341/341 for
...
# counter fidelity against the models themselves (341 samples x 5 counters, token level)
mkdir -p /tmp/tokenizer_oracle
for m in bge-m3 bge-large-en qwen3-embedding e5-mistral cl100k; do
  python3 scripts/gen_tokenizer_oracle.py --model $m --out /tmp/tokenizer_oracle/$m.json --fuzz 300
done
