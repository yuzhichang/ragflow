...
func checkCounterAgainstOracle(t *testing.T, fx oracleFixture) {
	...
	if !ok {
		t.Skipf("counter %q is not implemented yet", fx.Counter)
	}
	...
	for _, sample := range fx.Samples {
		...
		if sample.Label == "incident_span_42000_67000" {
			incident, incidentOracle = sample.Text, sample.Count
		}
	...
	t.Logf("agreement: %d/%d samples compared by %s (oracle: %s)", len(fx.Samples)-mismatches-approximated, len(fx.Samples), mode, fx.Oracle)
	if approximated > 0 {
		t.Logf("within the documented approximation on %d/%d samples (combining marks in a shape the served charmap normalizes differently - see the known_approx flag in scripts/gen_tokenizer_oracle.py - tolerance %d tokens)",
			approximated, len(fx.Samples), knownApproxTokenTolerance)
	}
	if fx.Crosscheck != "" {
		t.Logf("second reference: %s", fx.Crosscheck)
	}

	// The incident, pinned as a test: on the span document 78785.md failed on,
	// the model's tokenizer sees MORE than the 8192-token window while cl100k
	// sees less - which is why counting with cl100k alone let an oversized input
	// through and the provider answered 400/20015.
	if incident == "" || fx.Counter != CounterXLMRSentence {
		return
	}
	if incidentOracle <= 8192 {
		t.Errorf("the incident span counts %d tokens in %s, expected more than the 8192-token window", incidentOracle, fx.Model)
	}
	if cl100k := CountCL100K(); cl100k.Available() {
		if got := cl100k.Count(incident); got >= 8192 {
			t.Errorf("cl100k counts the incident span at %d tokens; the bug required it to be below 8192", got)
		} else {
			t.Logf("incident span: cl100k=%d (under the window) vs %s=%d (over it)", got, fx.Model, incidentOracle)
		}
	}
}
