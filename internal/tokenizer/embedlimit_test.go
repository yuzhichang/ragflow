...
func TestCalibrationRatchet(t *testing.T) {
	...
	// bound below 1: the bound is a safety margin, not an estimate.
	cal.ObserveUsage(key, 8182, 8027)
	if got := cal.RatioUpper(key); got != 1 {
		t.Fatalf("ratio after an under-counting observation = %v, want 1", got)
	}
...
func TestLimiterWithoutUsableCounter(t *testing.T) {
	...
	trimmed, tokens := limiter.Trim(text, 8192)
	limit := limiter.Limit(8192)
	if len(trimmed) > limit*4 {
		t.Fatalf("byte-level fallback kept %d bytes, want <= %d", len(trimmed), limit*4)
	}
	if tokens != limit {
...
func TestTrimByBytesKeepsRunesIntact(t *testing.T) {
	...
	got := trimByBytes(text, 10)     // 40-byte bound
	if len(got) > 40 {
		t.Fatalf("trimByBytes kept %d bytes, want <= 40", len(got))
	}
