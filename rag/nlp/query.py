...
import re
import logging
from utils.data_store_conn import MatchTextExpr
...
class FulltextQueryer:
    ...
    def question(self, txt, tbl="qa", min_match="60%"):
        ...
        if not self.isChinese(txt):
            ...
            return MatchTextExpr(
                self.query_fields, query, 100, {"min_match": min_match}
            ), tks
    ...
    def similarity(self, qtwt, dtwt):
        ...
        return (
            s
            / q
            / max(1, math.sqrt(math.log10(max(len(qtwt.keys()), len(dtwt.keys())))))
