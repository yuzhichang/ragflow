...
def task_chunks_exist(task: dict) -> (str, bool):
    ...
    task_kwd = md5.hexdigest()

    fields = ["id"]
    condition = {"task_kwd": task_kwd}
    ...
    return task_kwd, len(dict_chunks) > 0
...
def do_handle_task(task):
    ...
    task_kwd, found = task_chunks_exist(task)
    if found:
        progress_callback(1., msg=f"Chunks of task {task_kwd} already exist, skip.")
        return
