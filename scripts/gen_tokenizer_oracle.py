...
one is checked against the implementation that actually defines the answer:
  ...
  * xlmr-spm         - the HuggingFace `tokenizers` conversion of the model that
                       ...
                       canonical Google `sentencepiece` package cross-checked when
                       it is importable;
...
Usage:
    for m in bge-m3 bge-large-en qwen3-embedding e5-mistral cl100k; do
      python3 scripts/gen_tokenizer_oracle.py --model $m --out /tmp/tokenizer_oracle/$m.json
    done
...
# The document that exposed the bug: an English Wikipedia article about Argosy
# magazine with large volume/issue-number tables. The span below is the one that
# SILICONFLOW rejected with 400/20015 while cl100k counted it as *under* the
# 8192-token window.
INCIDENT_CORPUS = "/home/zhichyu/Downloads/data/browsecomp-plus/corpus/78785.md"
INCIDENT_SPAN = (42000, 67000)
...
def samples() -> list[tuple[str, str]]:
    ...
    if os.path.isfile(INCIDENT_CORPUS):
        with open(INCIDENT_CORPUS, encoding="utf-8") as fh:
            text = fh.read()
        start, end = INCIDENT_SPAN
        if len(text) > end:
            out.append(("incident_span_42000_67000", text[start:end]))
        out.append(("incident_head_2000", text[:2000]))
        out.append(("incident_full", text))
    else:
        print(f"note: {INCIDENT_CORPUS} not present, skipping the incident samples", file=sys.stderr)
    return out
...
def main() -> int:
    ...
    rows_src = samples() + (fuzz_samples(args.fuzz, args.seed) if args.fuzz else [])
    ...
    if rel is None:
        ...
        try:
            ...
            print("the `tiktoken` package is required for --model cl100k", file=sys.stderr)
            return 1
        ...
        try:
            ...
            print("the `tokenizers` package is required (pip install tokenizers)", file=sys.stderr)
            return 1
    ...
    if args.spm_crosscheck:
        ...
        try:
            ...
            crosscheck_note = "sentencepiece not installed: cross-check skipped (pip install sentencepiece to compare against Google's implementation)"
            print(f"note: {crosscheck_note}", file=sys.stderr)
        else:
            if not os.path.isfile(model_file):
                print(f"note: {model_file} missing, cross-check skipped", file=sys.stderr)
            else:
