...
async def run_graphrag(
    ...
    callback(f"GraphRAG for doc {doc_id} done in {now - start:.2f} seconds.")
    return
...
async def update_graph(
    ...
    contains = await trio.to_thread.run_sync(
        lambda: does_graph_contains(tenant_id, kb_id, doc_id)
    )
    if contains:
    ...
    ents, rels = await ext(chunks, callback)
    subgraph = nx.Graph()
    for en in ents:
    ...
    now = trio.current_time()
    callback(f"generated subgraph for doc {doc_id} in {now - start:.2f} seconds.")
    start = now
    ...
    now = trio.current_time()
    callback(
        f"merging subgraph for doc {doc_id} into the global graph done in {now - start:.2f} seconds."
    )
    return new_graph, now_docids


async def with_resolution(
    graph,
    doc_ids,
    tenant_id: str,
    kb_id: str,
    ...
    working_doc_id = graphrag_task_get(tenant_id, kb_id, doc_id)
    if doc_id != working_doc_id:
        callback(
            msg=f"Another graphrag task of doc_id {working_doc_id} is working on this kb, cancel myself"
        )
...
async def with_community(
    ...
    if doc_id != working_doc_id:
        ...
        return
    await trio.to_thread.run_sync(lambda: set_graph(tenant_id, kb_id, graph, doc_ids))

    now = trio.current_time()
    callback(
        msg=f"Graph community extraction done in {now - start:.2f}s.".format(
    ...
    now = trio.current_time()
    callback(
        msg=f"Graph community indexing {len(cr.structured_output)} communities done in {now - start:.2f}s."
    )
