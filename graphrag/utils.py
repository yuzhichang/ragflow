...
async def does_graph_contains(tenant_id, kb_id, doc_id):
    ...
    res = await trio.to_thread.run_sync(settings.dataStore.search(fields, [], condition, [], OrderByExpr(), 0, 1, search.index_name(tenant_id), [kb_id]))
    fields2 = settings.dataStore.getFields(res, fields)
    graph_doc_ids = set()
